...
export const TransactionRow = props => {
  ...
  return (
    <Row className={tx.code ? "tx-info invalid" : "tx-info"}>
      ...
      <Col className="resultpaddingleft" xs={!props.blockList ? 3 : 3} md={1}>
      ...
      <Col
        xs={!props.blockList ? 3 : 2}
        md={!props.blockList ? 2 : 2}
        className="fee"
      >
      ...
      {!props.blockList ? (
        <Col xs={2} md={2}>
          <span>
            {tx.block()
              ? moment.utc(tx.block().time).format("D MMM YYYY, h:mm:ssa")
              : ""}
          </span>
        </Col>
      ) : (
