...
const ValidatorRow = (props) => {
    ...
    return <Card body>
        <Row className="validator-info">
            ...
            <Col className="self-delegation data" xs={{size:4,offset:2}} md={{size:2,offset:0}}><i className="material-icons d-md-none screen">equalizer</i> <span>{props.validator.self_delegation?numbro(props.validator.self_delegation).format('0.00%'):'N/A'}</span></Col>
            {(!props.inactive)?<Col className="commission data" xs={{size:4}} md={{size:1,offset:0}} lg={2}><i className="material-icons d-md-none screen">call_split</i> <span>{numbro(props.validator.commission.rate).format('0.00%')}</span></Col>:''}
