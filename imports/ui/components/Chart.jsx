...
export default class PChart extends Component {
  ...
  createPlot(plotData) {
    ...
    if (plotData.attrs)
      plotData.attrs.forEach(attrData => {
        plot.attr(attrData.attr, attrData.value, this.getScale(attrData.scale));
    ...
    if (plotData.datasets) {
      plotData.datasets.forEach(datasetId => {
        ...
        allDatasetsHaveColor =
          allDatasetsHaveColor && !!dataset.metadata().color;
      });
    }
    ...
    switch (legendData.type) {
      case "Regular":
        ...
        if (legendData.domain && legendData.range) {
          ...
          ({ domain, range } = getColorDomainRangeFromPlots(
            legendData.plotIds
          ));
        }
  ...
  loadComponents() {
    ...
    let componentMeta = [
      ...
      { type: "labels", id: "labelId", createMethod: this.createLabel },
      {
        type: "gridlines",
        id: "ggridlineId",
        createMethod: this.createGridline
      },
  ...
  attachClickInteraction(interactionData, component, componentId) {
    ...
    for (let action in interactionData) {
      switch (action) {
        case "onClick":
          interaction.onClick(
            interactionData[action].bind(null, component, this)
          );
          break;
        case "onDoubleClick":
  ...
  attachKeyInteraction(interactionData, component, componentId) {
    ...
    for (let action in interactionData) {
      switch (action) {
        case "onKeyPress":
          interaction.onKeyPress(
            interactionData[action].bind(null, component, this)
          );
          break;
        case "onKeyRelease":
          interaction.onKeyRelease(
            interactionData[action].bind(null, component, this)
          );
          break;
        default:
  ...
  addTooltip(tooltipValue, component, componentId) {
    ...
    let tooltip = new Tooltip(selection.node(), {
      ...
      html: true,
      template:
        '<div class="tooltip bs-tooltip-auto" role="tooltip"><div class="tooltip-arrow"></div><div class="tooltip-inner"></div></div>'
    });
    ...
    interaction.onPointerMove(point => {
      ...
      if (closest) {
        ...
        tooltip.updateTitleContent(
          tooltipValue(component, point, closest.datum, closest.dataset)
        );
        tooltip.show();
      } else {
        tooltip.hide();
  ...
  componentDidMount() {
    ...
    this.addTooltipQueue.forEach(args => this.addTooltip.bind(this)(...args));
  }
    ...
    if (!_.isEqual(newScales, curScales)) {
      ...
      for (let scaleId in scalesMap) {
        ...
        if (!_.isEqual(newScale, curScale)) {
          if (
            newScale.type !== curScale.type ||
            (newScale.type == "InterpolatedColor" &&
              newScale.colorScale != curScale.colorScale)
          ) {
            this.scales[scaleId] = createScale(newScales[newId]);
    ...
    for (let plotId in plotsMap) {
      ...
      if (newPlot.type !== "Pie") {
        ...
        if (!_.isEqual(newPlot.sectorValue, curPlot.sectorValue)) {
          plot.sectorValue(
            newPlot.sectorValue.value,
            this.getScale(newPlot.sectorValue.scale)
          );
        } else if (
          curPlot.sectorValue.scale &&
          _.has(scalesDiff, curPlot.sectorValue.scale)
        ) {
          plot.sectorValue(
            curPlot.sectorValue.value,
            this.getScale(curPlot.sectorValue.scale)
          );
        }
      }
  ...
  updateLegends(curLegends = [], newLegends = [], scalesDiff, componentsDiff) {
    ...
    for (let legendId in legendsMap) {
      ...
      if (
        curId == undefined ||
        newLegends[newId].type != curLegends[curId].type
      ) {
      ...
      let newLegend = newLegend[newId],
        curLegend = LegendAxes[curId];
      if (newLegends[newId].type == "Regular") {
        if (newLegends.domain && newLegends.range) {
          ...
          let { domain, range } = getColorDomainRangeFromPlots(
            newLegends.plotIds
          );
  ...
  updateLabels(curLabels = [], newLabels = [], scalesDiff, componentsDiff) {
    ...
    for (let labelId in labelsMap) {
      ...
      if (
        curId == undefined ||
        newLabels[newId].type != curLabels[curId].type
      ) {
      ...
      let newLabel = newLabel[newId],
        curLabel = LabelurAxes[curId];
  ...
  updateGridlines(
    ...
    for (let gridlineId in gridlinesMap) {
      ...
      let newGridline = newGridline[newId],
        curGridline = curGridline[curId];
      if (
        newGridline.xScale !== curGridline.xScale ||
        _.has(scalesDiff, curGridline.xScale)
      ) {
        gridline.xScale = this.getScale(newGridline.xScale);
      }
      if (
        newGridline.yScale !== curGridline.yScale ||
        _.has(scalesDiff, curGridline.yScale)
      ) {
  ...
  updateGroups(curGroups = [], newGroups = [], scalesDiff, componentsDiff) {
    ...
    for (let groupId in groupsMap) {
      ...
      if (newId == undefined) {
        ...
        curId == undefined ||
        newGroups[newId].type != curGroups[curId].type
      ) {
  ...
  updateLayout(newLayout, curLayout, componentsDiff) {
    ...
    curLayout.forEach((row, i) =>
      row.forEach((curCompId, j) => {
